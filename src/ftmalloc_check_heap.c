...
static t_bool   bin_verify_contiguous_internal(t_mem_bin *bin)
{
	...
	while (bin)
	{
		...
		{
			max_chunk_sz = FTMALLOC_BIN_ITEM_MAX_ALLOC_SIZE_SMALL - FTMALLOC_CHUNK_SZ;
		...
		{
			max_chunk_sz = FTMALLOC_BIN_ITEM_MAX_ALLOC_SIZE_MEDIUM - FTMALLOC_CHUNK_SZ;
...
int		    ftmalloc_check_heap_fully(void)
{
	...
	if (!is_valid)
	{
		...
		FTMALLOC_ASSERT("ftmalloc heap corruption error: contiguous memory has been corrupted!");
	}
