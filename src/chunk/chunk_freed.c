...
t_mem_chunk	*chunk_freed_prev_get(const t_mem_chunk *chunk)
{
	...
	FTMALLOC_ASSERT(!freed_prev || !chunk_in_use_get(freed_prev));
	return (freed_prev);
}
...
t_mem_chunk	*chunk_freed_next_get(const t_mem_chunk *chunk)
{
	...
	FTMALLOC_ASSERT(!freed_next || !chunk_in_use_get(freed_next));
	return (freed_next);
}
...
void		chunk_freed_prev_set(t_mem_chunk *chunk, t_mem_chunk *freed)
{
	...
	FTMALLOC_ASSERT(!chunk_in_use_get(chunk));
	FTMALLOC_ASSERT(!freed || !chunk_in_use_get(freed));
